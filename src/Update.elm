...
init _ url key =
    singleton Model |> andMapCmd MsgForRouter (Router.Update.init url key) |> andMapCmd MsgForCats Cats.Update.init |> andMapCmd MsgForCounter Counter.Update.init |> andMapCmd MsgForWallet Wallet.Update.init |> andMapCmd MsgForExplorer Explorer.Update.init


update : Msg -> (Model -> Return Msg Model)
update msg model =
    singleton Model |> andMapCmd MsgForRouter (Router.Update.update msg model.router) |> andMapCmd MsgForCats (Cats.Update.update msg model.cats) |> andMapCmd MsgForCounter (Counter.Update.update msg model.counter) |> andMapCmd MsgForWallet (Wallet.Update.update msg model.wallet) |> andMapCmd MsgForExplorer (Explorer.Update.update msg model.explorer)
